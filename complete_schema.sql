-- =============================================================================
-- The Royal Passage — complete_schema.sql
-- Single deployable schema (all migrations reconciled into one file)
-- =============================================================================
...
--
-- Reconciled from: supabase/FULL_SCHEMA.sql + supabase/migrations/*
--   (duplicates removed; later migrations win; safe to re-run)
--
-- Includes:
...
--   • Multi-role access (user_roles) + signup role sync + staff role repair
...
--   • Homestay extras: weekend pricing, extra beds, rejection reason, decision contact
--   • VIP owner module + guest membership applications + custom package requests
--   • Partner experience / homestay applications (KYC + approve provisioning)
--   • Guest cancel-abuse freeze, escalation contacts, compare-at offer pricing
--   • Admin pending-1h flags, host booking / upcoming reminder email tracking
...
-- user_roles sync is applied later once public.user_roles exists.
...
-- =============================================================================
-- RECONCILED MIGRATIONS (missing from earlier FULL_SCHEMA consolidation)
-- Chronological: 20260710 → 20260716 (+ small earlier gap: paused index)
-- =============================================================================

-- Booking pause partial index (20250612)
create index if not exists idx_bookings_is_paused on public.bookings (is_paused)
  where is_paused = true;

-- Homestay booking rejection reason (20260710)
alter table public.homestay_bookings
  add column if not exists rejection_reason text;

-- Multi-role access (20260710)
...
-- Signup sync: profile + user_roles (20260714)
create or replace function public.handle_new_user()
returns trigger
language plpgsql
security definer
...
-- Repair staff accounts left as guest after provisioning races (20260714)
...
-- Admin pending-1h notification flags (20260715)
alter table public.bookings
...
-- Host/owner decision contact + experience rejection reason (20260715)
alter table public.bookings
...
-- Guest book+cancel abuse freeze (20260715)
alter table public.profiles
...
-- Partner experience applications (20260715 + KYC fields + trade licence expiry)
create table if not exists public.partner_experience_applications (
  ...
  created_experience_id uuid references public.experiences (id) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
alter table public.partner_experience_applications
  ...
  add column if not exists created_experience_id uuid;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'partner_experience_applications_created_user_id_fkey'
  ) then
    alter table public.partner_experience_applications
      add constraint partner_experience_applications_created_user_id_fkey
      foreign key (created_user_id) references auth.users (id) on delete set null;
  end if;
  if not exists (
    select 1 from pg_constraint where conname = 'partner_experience_applications_created_experience_id_fkey'
  ) then
    alter table public.partner_experience_applications
      add constraint partner_experience_applications_created_experience_id_fkey
      foreign key (created_experience_id) references public.experiences (id) on delete set null;
  end if;
end $$;
...
-- Partner homestay applications + KYC (20260715)
...
-- Final notification types (includes partner + freeze + overdue)
alter table public.notifications drop constraint if exists notifications_type_check;
alter table public.notifications
  add constraint notifications_type_check check (type in (
    'booking_created', 'booking_confirmed', 'booking_cancelled',
