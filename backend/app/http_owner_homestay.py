...
from app.services.owner_homestay_bookings import get_owner_homestay_revenue, list_owner_homestay_reviews
...
async def owner_homestay_reviews(request: Request) -> JSONResponse:
    auth = require_homestay_owner_request(request)
    if isinstance(auth, JSONResponse):
        return auth
    try:
        reviews = list_owner_homestay_reviews(auth)
