...
export async function uploadVipAadhaarPhoto(file: File): Promise<string> {
  ...
  if (!user) {
    throw new Error("Sign in before uploading your Aadhaar photo.");
  }

  const ext = extensionForFile(file);
  const path = `${user.id}/vip-aadhaar-${Date.now()}-${crypto.randomUUID().slice(0, 8)}.${ext}`;
