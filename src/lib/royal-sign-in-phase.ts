...
export function getRoyalSignInPhaseFlags(phase: RoyalSignInPhase) {
  return {
    ...
    cameraForward: ["forward", "courtyard", "particles", "logo", "done"].includes(phase),
    showCourtyard: ["courtyard", "particles", "logo", "done"].includes(phase),
    showTrail: ["particles", "logo"].includes(phase),
    doorsFlooding: phase === "doors-open" || phase === "forward",
    logoGlow: phase === "logo",
    logoPulse: phase === "logo",
    unfolding: false,
    isExiting: phase === "logo" || phase === "done",
