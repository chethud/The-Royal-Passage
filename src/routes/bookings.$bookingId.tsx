...
import { cancelMyBooking, getBookingDetail, type BookingSummary } from "@/lib/booking-fns";
...
function BookingDetailPage() {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const updated = await cancelMyBooking({ data: { accessToken: token, bookingId } });
      setBooking(updated);
    } catch (err) {
      setLoadError(err instanceof Error ? err.message : "Failed to cancel booking.");
    } finally {
      setCancelling(false);
    }
  };
