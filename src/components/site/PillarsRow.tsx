...
export function PillarsRow() {
  return (
    <section className="border-t border-[oklch(0.88_0.08_86_/_0.12)] bg-[color-mix(in srgb, var(--brand-noir) 88%, var(--brand-maroon) 12%)] pt-5 pb-4 sm:pt-10 sm:pb-7 md:pt-12 md:pb-8">
      <div className="container-page px-1 sm:px-6">
        <div className="grid grid-cols-4 gap-1.5 sm:gap-4 md:gap-6 lg:gap-8">
          {pillars.map((p, idx) => {
            const Icon = p.icon;
            return (
              <ScrollReveal
                ...
                offsetY={18}
                className={cn(
                  "relative flex min-w-0 flex-col items-center px-0.5 text-center sm:px-3 md:px-4",
                  ...
                  idx > 0 && "before:block after:block",
                )}
              >
                <span className="mb-2 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-ember/55 bg-ember/8 text-ember shadow-[inset_0_1px_0_oklch(0.92_0.06_82_/_0.18)] sm:mb-5 sm:h-16 sm:w-16">
                  <Icon className="h-4.5 w-4.5 sm:h-9 sm:w-9" />
                </span>
                <div className="font-display text-[0.58rem] uppercase leading-tight tracking-[0.06em] text-ink sm:text-[0.95rem] sm:tracking-[0.2em]">
                  {p.title}
                </div>
                <p className="mt-1 max-w-[14rem] text-[0.52rem] leading-snug text-muted-foreground sm:mt-2 sm:max-w-[16rem] sm:text-[0.78rem] sm:leading-relaxed">
                  {p.description}
                </p>
              </ScrollReveal>
            );
