...
type TravelAgentDashboardShellProps = {
  title: string;
  subtitle: string;
  children: ReactNode;
...
export function TravelAgentDashboardShell({
  title,
  subtitle,
  children,
  ...
  const isRoyalHero =
    variant === "overview" || variant === "bookings" || variant === "catalog";
        ...
        {isRoyalHero ? (
          <header
            className={`host-overview-hero ${
              variant === "bookings" || variant === "catalog" ? "host-overview-hero--bookings" : ""
            }`}
          >
            <div
            ...
            <div className="relative z-[1]">
              <div className="mb-4 flex flex-wrap items-center gap-3 sm:mb-5">
                ...
                <span className="text-[0.68rem] font-semibold uppercase tracking-[0.2em] text-[rgb(212_175_55/0.72)]">
                  {ROLE_LABELS.travel_agent} dashboard
                </span>
              </div>
              ...
              {showRoleDescription ? (
                <p className="mt-2 max-w-xl text-[0.68rem] uppercase tracking-[0.12em] text-[rgb(212_175_55/0.58)]">
                  {ROLE_DESCRIPTIONS.travel_agent}
                </p>
              ) : null}
            </div>
          </header>
          ...
          <>
            <div className="mb-6 flex flex-wrap items-center gap-3 sm:mb-8">
              <RoleBadge role="travel_agent" />
              <span className="text-sm text-muted-foreground">{ROLE_LABELS.travel_agent} dashboard</span>
            </div>
        ...
        <div
          className={
            variant === "overview"
              ...
              : variant === "bookings" || variant === "catalog"
